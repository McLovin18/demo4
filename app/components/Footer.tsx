...
const socialLinks = [
  { href: "https://www.instagram.com/alegria_creativecakes/", label: "Instagram", Icon: IconInstagram },
];

// 👉 Información del negocio
const WHATSAPP_NUMBER = "593999391279"; // solo números, con código de país, sin '+' ni espacios
const WHATSAPP_DISPLAY = "+593 99 939 1279"; // como se muestra al usuario
      ...
      <footer className=" border-t border-white/10 text-white">
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left gap-1">
              ...
              <div className="text-xs text-white/60 mt-1 max-w-[220px]">
                <p>Tortas temáticas, postres y dulces para eventos</p>
                <p>Diseños Personalizados • Hecho con Amor</p>
                <p className="flex items-center gap-1 justify-center md:justify-start mt-0.5">
                  <IconLocation />
                  Ecuador · Envíos a todo lugar
                </p>
              </div>
            </div>
                ...
                <ul className={styles.ftSocials}>
                  {socialLinks.map(({ href, label, Icon }) => (
                    <li key={label}>
                      <a
                        href={href}
                        className="flex items-center justify-center w-9 h-9 rounded-full border border-white/15 text-white transition-colors hover:bg-[#E0568F] hover:border-[#E0568F]"
                        target="_blank"
                        rel="noreferrer"
                        title={label}
            ...
            <div className="flex flex-col items-center md:items-end gap-2.5">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center gap-2 text-xl text-white/70 hover:text-[#E0568F] transition-colors"
                onClick={() => trackLinkClick().catch(console.error)}
              >
        ...
        <div className={styles.ftCopyRow}>
          <p className="text-xs text-white/50">
            © {new Date().getFullYear()} Alegría Creative Cakes. Todos los derechos reservados.
          </p>
          <div className={styles.ftCopyRight}>
            <div className="flex items-center gap-1.5 text-xs text-white/60">
              <div className="w-1.5 h-1.5 rounded-full bg-[#E0568F]" />
              Hecho en Ecuador
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs text-white/50 hover:text-[#E0568F] transition-colors"
              onClick={() => trackLinkClick().catch(console.error)}
            >
